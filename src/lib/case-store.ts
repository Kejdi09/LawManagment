import { AuditLogRecord, Case, CaseState, CaseTask, CommEntry, Customer, CustomerHistoryRecord, CustomerNotification, DeletedRecord, HistoryRecord, Invoice, InvoicePayment, Meeting, Note, PortalData, PortalNote, SearchResult, TeamSummary } from "./types";
...
export async function selectPortalPaymentMethod(token: string, method: 'bank' | 'crypto' | 'cash'): Promise<void> {
  const res = await fetch(`${API_URL}/api/portal/${encodeURIComponent(token)}/select-payment`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function markPaymentDone(
  customerId: string,
  opts?: { initialPaymentAmount?: number; currency?: string; invoiceId?: string }
): Promise<void> {
  await api(`/api/customers/${customerId}/mark-payment-done`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function getCustomersAwaitingPayment(): Promise<import('./types').Customer[]> {
  return api<import('./types').Customer[]>('/api/customers/awaiting-payment');
}

export async function setInitialPaymentAmount(
  customerId: string,
  amount: number,
  currency: string
): Promise<void> {
  await api(`/api/customers/${customerId}/initial-payment-amount`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
